...
# Created:       2009-07-10
# Last Modified: $Date$
# $HeadURL: https://proserver.svn.sourceforge.net/svnroot/proserver/branches/1.6-spec/lib/Bio/Das/ProServer/SourceAdaptor/Transport/martservice.pm $
#
...
sub attributes {
    ...
    if ( $attributes ) {
        if ( ref( $attributes ) ) {
            ...
            $self->{'attributes'} = [ split qr(\s*,\s*), $attributes ];
        }
    }
...
sub build_query_xml {
    ...
    die 'No attributes defined'
        unless $self->attributes and @{ $self->{attributes} };
        
    die 'No filter defined'
        unless $filter;
...
sub query {
    ...
    die 'No martservice defined'
        unless $self->martservice;
